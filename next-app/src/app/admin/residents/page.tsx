...
import type { Society, House, Resident, User } from '@/types'
...
function previewPassword(houses: SampleHouse[], houseId: string): string {
...
function CredentialsPopup({ creds, onClose }: CredentialsPopupProps) {
...
interface SampleResident extends Resident {
  name: string
  phone: string | null
  house: SampleHouse
  user: SampleUser
...
const makeSampleData = () => {
  const housesBySociety: Record<string, SampleHouse[]> = {}
  const residentsBySociety: Record<string, SampleResident[]> = {}

  housesBySociety['soc-1'] = [
    { id: 'h1', houseNo: '1', floor: 'Ground', isActive: true, createdAt: new Date(), updatedAt: new Date(), societyId: 'soc-1', society: {} as any, resident: null, billEntries: [] },
    { id: 'h2', houseNo: '2', floor: 'Ground', isActive: true, createdAt: new Date(), updatedAt: new Date(), societyId: 'soc-1', society: {} as any, resident: null, billEntries: [] },
    { id: 'h3', houseNo: '3', floor: 'Ground', isActive: true, createdAt: new Date(), updatedAt: new Date(), societyId: 'soc-1', society: {} as any, resident: null, billEntries: [] },
    { id: 'h10', houseNo: '10', floor: 'First', isActive: true, createdAt: new Date(), updatedAt: new Date(), societyId: 'soc-1', society: {} as any, resident: null, billEntries: [] },
    { id: 'h11', houseNo: '11', floor: 'First', isActive: true, createdAt: new Date(), updatedAt: new Date(), societyId: 'soc-1', society: {} as any, resident: null, billEntries: [] },
    { id: 'h12', houseNo: '12', floor: 'Second', isActive: true, createdAt: new Date(), updatedAt: new Date(), societyId: 'soc-1', society: {} as any, resident: null, billEntries: [] },
  ]

  housesBySociety['soc-2'] = [
    { id: 'hs2-1', houseNo: 'A-101', floor: 'Ground', isActive: true, createdAt: new Date(), updatedAt: new Date(), societyId: 'soc-2', society: {} as any, resident: null, billEntries: [] },
    { id: 'hs2-2', houseNo: 'A-102', floor: 'Ground', isActive: true, createdAt: new Date(), updatedAt: new Date(), societyId: 'soc-2', society: {} as any, resident: null, billEntries: [] },
  ]

  residentsBySociety['soc-1'] = [
    {
      id: 'r1',
      name: 'Rajesh Patel',
      phone: '9876543210',
      ...
      updatedAt: new Date(),
      houseId: 'h1',
      house: housesBySociety['soc-1'][0],
      userId: 'u1',
      user: { id: 'u1', email: 'rajesh@gmail.com', passwordHash: '', role: 'RESIDENT', isActive: true, createdAt: new Date(), updatedAt: new Date(), adminSocieties: [], resident: null }
    },
    {
      id: 'r2',
      name: 'Amit Shah',
      ...
      updatedAt: new Date(),
      houseId: 'h2',
      house: housesBySociety['soc-1'][1],
      userId: 'u2',
      user: { id: 'u2', email: 'amit@gmail.com', passwordHash: '', role: 'RESIDENT', isActive: true, createdAt: new Date(), updatedAt: new Date(), adminSocieties: [], resident: null }
    },
    {
      id: 'r10',
      name: 'Resident 10',
      ...
      updatedAt: new Date(),
      houseId: 'h10',
      house: housesBySociety['soc-1'][3],
      userId: 'u10',
      user: { id: 'u10', email: 'resident10@pathiksco.com', passwordHash: '', role: 'RESIDENT', isActive: true, createdAt: new Date(), updatedAt: new Date(), adminSocieties: [], resident: null }
    }
  ]

  residentsBySociety['soc-2'] = [
    {
      id: 'rs2-1',
      name: 'Suresh Kumar',
      phone: '9876543300',
      ...
      updatedAt: new Date(),
      houseId: 'hs2-1',
      house: housesBySociety['soc-2'][0],
      userId: 'us2-1',
      user: { id: 'us2-1', email: 'suresh@gmail.com', passwordHash: '', role: 'RESIDENT', isActive: false, createdAt: new Date(), updatedAt: new Date(), adminSocieties: [], resident: null }
    }
  ]

  residentsBySociety['soc-1'][0].house.resident = residentsBySociety['soc-1'][0]
  residentsBySociety['soc-1'][1].house.resident = residentsBySociety['soc-1'][1]
  residentsBySociety['soc-1'][2].house.resident = residentsBySociety['soc-1'][2]
  residentsBySociety['soc-2'][0].house.resident = residentsBySociety['soc-2'][0]

  return { housesBySociety, residentsBySociety }
}
...
export default function ResidentManagement() {
  ...
  const [residents, setResidents] = useState<SampleResident[]>([])
  const [showForm, setShowForm] = useState(false)
  const [resetTarget, setResetTarget] = useState<string | null>(null)
  const [newPassword, setNewPassword] = useState('')
  const [showNewPwd, setShowNewPwd] = useState(false)
  const [createdCreds, setCreatedCreds] = useState<GeneratedCredentials | null>(null)
  ...
  const availableHouses = useMemo(() => houses.filter(h => !h.resident), [houses])
  ...
  const loadData = useCallback(async (socId: string) => {
    if (!socId) { setHouses([]); setResidents([]); return }
    setLoadingData(true)
    try {
      const [housesRes, residentsRes] = await Promise.all([
        fetch(`/api/houses?societyId=${encodeURIComponent(socId)}`),
        fetch(`/api/residents?societyId=${encodeURIComponent(socId)}`),
      ])
      if (housesRes.ok) {
      ...
      if (residentsRes.ok) {
        const r = await residentsRes.json()
        setResidents(r.residents || [])
        const byHouse: Record<string, SampleResident> = {}
        ;(r.residents || []).forEach((rv: SampleResident) => { byHouse[rv.houseId] = rv })
        setHouses(prev => prev.map(h => ({ ...h, resident: byHouse[h.id] || null })))
      }
    } catch {
  ...
  useEffect(() => { if (selectedSociety) loadData(selectedSociety) }, [selectedSociety, loadData])
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const email = previewEmail(form.name)
      const password = selectedHouse.houseNo
      ...
      const data = await res.json()
      setCreatedCreds({ email: data.credentials?.email || email, password: data.credentials?.password || password })
      setShowForm(false)
      setForm({ name: '', phone: '', houseId: '' })
      loadData(selectedSociety)
    } catch (err: any) {
      toast.error(err.message || 'Error creating resident')
    } finally {
      setCreating(false)
    }
  }
  ...
  const handleResetPassword = async () => {
    ...
    try {
      const res = await fetch(`/api/residents/${resetTarget}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
